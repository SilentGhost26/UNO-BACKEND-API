...
const gameMustBeWaiting = ({game, gameId}) => {
    if (game.status === 'PLAYING') {
        return conflictHelper.throwError409(`game with ID ${gameId} already playing`);
    }
...
const gameMustMimimumPlayers = ({currentPlayers, gameId}) => {
    if (currentPlayers.length < 2) {
        return conflictHelper.throwError409(`game with ID ${gameId} doesn't have enough players`);
    }
    return ok();
}
